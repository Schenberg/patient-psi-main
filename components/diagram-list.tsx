...
export function DiagramList({ userId, chatId, patientProfile: propPatientProfile }: DiagramListProps) {
    const [patientProfile, setPatientProfile] = useState<PatientProfile | null>(propPatientProfile || initialProfile);
    const [isFetchedPatientProfile, setIsFetchedPatientProfile] = useState(!!propPatientProfile);
    ...
    const [CCDResults, setCCDResults] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`/api/ccd?userId=${userId}&chatId=${chatId}`);
                if (!response.ok) {
