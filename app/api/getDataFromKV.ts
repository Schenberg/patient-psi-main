...
export async function sampleProfile(): Promise<PatientProfile | null> {
    try {
        console.log('Starting sampleProfile');
        const all_keys = await kv.keys('profile_*');
        console.log('Found profile keys:', all_keys);

        if (all_keys.length === 0) {
            console.log('No profiles found in KV');
            return null;
        }

        const randomIndex = Math.floor(Math.random() * all_keys.length);
        const randomKey = all_keys[randomIndex];
        console.log('Selected random key:', randomKey);

        const profileData = await kv.get(randomKey);
        console.log('Retrieved profile data:', profileData);

        if (!profileData) {
            console.log('No profile data found for key:', randomKey);
            return null;
        }
